        ...
        private void btnIN_Click(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txbTienNhan.Text, out tiennhan) == true)
            {
                if (int.Parse(txbThanhTien.Text) < tiennhan)
                {
                    ...
                    HoaDonDAO.Instance.ThemHoaDon(mahd, dtpNgay.Value.Date);
                    if (dtgHoaDon.Rows.Count > 1)
                    {
                        foreach (DataGridViewRow row in dtgHoaDon.Rows)
                        {
                            if (row.Index < dtgHoaDon.Rows.Count - 1)
                            {
                                ...
                                HoaDonDAO.Instance.ThemChiTietHD(mahd, row.Cells[0].Value.ToString(), int.Parse(row.Cells[2].Value.ToString()), int.Parse(row.Cells[3].Value.ToString()));
                            }
                        }
                    }
                    InHoaDon.Print();
